...
class PrayerTimeWidgetProvider : AppWidgetProvider() {
    ...
    private fun updateWidget(
        ...
        // Hedef zamanı al (Flutter'dan kaydedilen timestamp - String olarak)
        val prayerEndTimeStr = widgetData.getString("prayer_end_time", "0")
        val prayerEndTime = try {
            prayerEndTimeStr?.toLong() ?: 0L
        } catch (e: Exception) {
            0L
        }
        ...
        // Çember görselini oluştur (sadece grafik, metin yok)
        ...
        val views = RemoteViews(context.packageName, R.layout.prayer_time_widget).apply {
            ...
            // Metinleri ayarla
            setTextViewText(R.id.next_prayer_label, nextPrayerLabel)
            setTextViewText(R.id.next_prayer_name, nextPrayer)
            setTextViewText(R.id.remaining_label, remainingLabel)

            // Chronometer ayarla
            if (prayerEndTime > System.currentTimeMillis()) {
                // SystemClock.elapsedRealtime() bazlı zamanı hesapla
                val elapsedRealtimeOffset = SystemClock.elapsedRealtime() - System.currentTimeMillis()
                val baseTime = prayerEndTime + elapsedRealtimeOffset
                
                setChronometer(R.id.prayer_timer, baseTime, null, true)
                setChronometerCountDown(R.id.prayer_timer, true)
                
                // Geri sayım bitince güncellenmesi için alarm kur (2 saniye gecikmeli)
                WidgetUpdateReceiver.scheduleNextUpdate(context, prayerEndTime + 2000)
            } else {
                // Süre dolmuşsa 00:00 göster ve hemen güncelleme iste
                setChronometer(R.id.prayer_timer, SystemClock.elapsedRealtime(), null, false)
                WidgetUpdateReceiver.scheduleNextUpdate(context, System.currentTimeMillis() + 1000)
            }
        }
    ...
    private fun calculateAngles(prayerTimes: List<String>, maghribTime: String): List<Float> {
        ...
        return prayerTimes.map { time ->
            ...
            (diff.toFloat() / totalMinutes) * 360f - 90f
        }
    }
