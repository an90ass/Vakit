...
class MyPrayersBloc extends Bloc<MyPrayersEvent, MyPrayersState> {
  ...
  Future<void> _getPrayerDay(
    String dateKey,
    Emitter<MyPrayersState> emit,
  ) async {
    emit(MyPrayersLoading());
    debugPrint('Loading prayer data for $dateKey');
    try {
      final prayerDay = PrayerStorage.getPrayerDay(dateKey);
      if (prayerDay != null) {
        debugPrint('Prayer data found: $prayerDay');
  ...
  Future<void> _updatePrayerDay(
    UpdatePrayerStatus event,
    Emitter<MyPrayersState> emit,
  ) async {
    ...
    if (currentState is MyPrayersLoaded) {
      try {
        await PrayerStorage.updatePrayerStatus(
          event.dateKey,
          event.prayerName,
          event.status,
        );
