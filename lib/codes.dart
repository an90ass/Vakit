...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  final List<String> prayerNames = const ['İmsak','Güneş','Öğle','İkindi','Akşam','Yatsı'];
 ...
 // Basit uyarı metinleri (ayet/hadis anlamlı ikazlar)
  ...
  Future<void> _updateHijriDate() async {
    ...
    final url = Uri.parse('https://api.aladhan.com/v1/gToH?date=${d.day}-${d.month}-${d.year}');
    try {
      ...
      const map = {
        'محرم': 'Muharrem','صفر': 'Safer','ربيع الأول': 'Rebiülevvel','رَبيع الثاني': 'Rebiülahir',
        'جمادى الأولى': 'Cemaziyelevvel','جمادى الآخرة': 'Cemaziyelahir','رجب': 'Recep','شعبان': 'Şaban',
        'رمضان': 'Ramazan','شوال': 'Şevval','ذو القعدة': 'Zilkade','ذو الحجة': 'Zilhicce',
      };
      setState(() {
  ...
  List<double> _angles(){
    ...
    final aksamMins = _parseToday(prayerTimes!['Akşam']!).difference(dayStart).inMinutes;
    const total = 24*60;
  ...
  void _scheduleHijriMidnightUpdate(){
    ...
    final nextMidnight = DateTime(n.year, n.month, n.day).add(const Duration(days: 1));
